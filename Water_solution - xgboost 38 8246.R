...
#Solution is 10th out of 2017 participants

#I used the XGBoost package in R to train the model which uses a gradient boosting framework.
#I used an ensemble of 11 XGBoost models with the same parameters and chose the most frequent 
#classification from the 11 models for each prediction.

#To develop a predictive model, I first removed duplicate variables.
#The original data set contained 40 variables. I reduced it down to 26 variables by removing variables
#that were similar/duplicates of other variables (e.g. there were many location variables).
...
#XXXXXX
#
#Time elapsed to run model: user - 1106.82  system - 59.08 elapsed - 455.91
...
#Set region_code and district_code as a factor
...
#For loop to run model 11 time with different random seeds
...
xgb.tab = xgb.cv(data = train.DMatrix, objective = "multi:softmax", booster = "gbtree",
                 nrounds = 50, nfold = 6, early.stop.round = 10, num_class = 4, maximize = FALSE,
...
#Add the solution to column i of the solutions data frame
...
x=1
#Finds the element that has the most votes for each row
...
write.csv(solution, file = "Water_solution - xgboost 38.csv", row.names = FALSE)

#Calculate the importance of each variable to the model.
#Used this function to remove variables from the model. Tried to make the model as simple as
#possible by including as few variables as possible
